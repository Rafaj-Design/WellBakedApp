	...
	$("a#submit").click(function(){
		...
		$.ajax({
			...
			success: function(){
				$('.error').fadeOut(0);
				$('form#contact-form').clearForm();
				$('#contact_form').html("<div>Contact form submitted!</div>").append("<br><label for='message'><strong>We will be in touch soon.</strong></label>").fadeOut(0).fadeIn(700, function(){
					$('#contact_form').append("<br><br><a id='back' onclick='window.location.reload(); return false;' class='button-5'>back</a>");
				});
			}
		});				
		return false;
